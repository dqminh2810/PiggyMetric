...
until mongosh $MONGO_INITDB_DATABASE --eval "$create_user" || mongosh $MONGO_INITDB_DATABASE $auth --eval "$create_user"; do sleep 5; done
killall mongod
sleep 1
killall -9 mongod
) &
...
if test -n "$INIT_DUMP"; then
	...
	until mongosh $MONGO_INITDB_DATABASE $auth $INIT_DUMP; do sleep 5; done
fi
) &
